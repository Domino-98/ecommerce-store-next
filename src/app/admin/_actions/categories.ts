...
import { deleteImage, utapi } from "@/lib/uploadthing";
import { Category, DrizzleError } from "@/lib/database/types";
...
export async function editCategory(formData: FormData, isImageChanged: boolean = false, currentCategory: Category) {
    const name = formData.get("name") as string;
    const description = formData.get("description") as string;
    const slug = formData.get("slug") as string;
    const parentId = formData.get("parentId") as string | null;
    ...
    try {
        ...
        await db.update(categoryTable).set({ name, description, slug, parentId: parentId ? parentId : null, image: isImageChanged ? imageUrl : currentCategory.image }).where(eq(categoryTable.id, currentCategory.id))
